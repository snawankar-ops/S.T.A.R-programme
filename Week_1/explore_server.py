...
class ExploreForwardServer(Node):
    ...
    def __init__(self):
        ...
        self.vel_pub.publish(Twist()) # publish a zero-velocity command to ensure the robot is stationary when we start
    ...
    def odom_callback(self, odom_msg: Odometry):
        ...
        self.posx = odom_pose.position.x
        self.posy = odom_pose.position.y
    ...
    def lidar_callback(self, scan_msg: LaserScan):
        ...
        front_left = scan_msg.ranges[0:21]
        front_right = scan_msg.ranges[-20:]
    ...
    def goal_callback(self, goal: ExploreForward.Goal):
        ...
        if goal.fwd_velocity < 0 or goal.fwd_velocity > 1.0:
            self.get_logger().warn(
                "Invalid fwd_velocity! Select a value between 0 and 1 m/s."
            )
            goal_ok = False
        ...
        # close!)
        if goal.stopping_distance < 0.2 or goal.stopping_distance > 10.0:
            self.get_logger().warn(
                "Invalid stopping_distance! Select a value between 0.2 and 10.0 m."
            )
            goal_ok = False
